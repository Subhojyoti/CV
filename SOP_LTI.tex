...
\textbf{1. Intelligent Tutoring Systems:} The task of designing an intelligent tutoring system that adapts to the changing nature of the student/viewer is a challenging task. One of the main challenges is that if the system keeps on giving very trivial task or if the system gives very challenging task then the student might lose interest and leave the task. So, the system must give a sequence of tasks that closely adapting the learning curve of the student, maybe varying the difficulty level slowly. Note, the sequential nature of the tutoring system and the exploration-exploitation dilemma it faces, and to this I can contribute heavily.

\textbf{2. Prediction from Patient EHR data:} This is an area that is not only challenging but directly sits across two of my most passionate areas of research, Safe Reinforcement Learning and learning text representations. Note that a typical EHR patient health record consist of a series of interactions of the patients with physicians, resulting in multiple electronic prescriptions (text) for the patient and their feedback. The first challenge which is slightly easier to solve is to find correlation of diseases and administered/suggested drug in one such prescription. The more challenging task is to connect many of such prescription generated sequentially over time and give some long term prediction with high guarantee. Again, I can draw from my experience in RL and learning theory to contribute to this. Currently one of my research project is focusing on predictions from the MIMIC 3 dataset containing such EHR of patients.


\textbf{3. Text-based Game playing:} In text-based game the main challenge lies in the ability to learn optimal policies in systems where action
space is defined by sentences in natural language, continuous state space and 
games with multiple endings and rewards. These feedbacks allows us to employ reinforcement learning techniques and jointly learn text representations and control policies. Again, the main challenge lies in constructing text representations that captures the long term correlations of actions and employing RL to modify the learned policy.
